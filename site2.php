...
<script type="text/javascript">
	...
	function setupTraffic(){
		...
		//var url	= "http://bbc.blueghost.co.uk/includes/travel_new.php"; //get xml file direct now
	...
	function reload(){
		...
		if (weather){
			...
			nav += '&nbsp;&raquo;&nbsp;<select onchange="javascript:goTo(this.options[this.selectedIndex].value);"><option value="weather" selected="true">Weather</option><option value="rail">Railways</option><option value="traffic">Roads</option></select>';
			
		}
		if (rail){
			var nav = '<a href="about.html" title="About Page">About</a> | Displaying Current Delays on the National Rail Network';
			nav += '&nbsp;&raquo;&nbsp;<select onchange="javascript:goTo(this.options[this.selectedIndex].value);"><option value="weather">Weather</option><option value="rail" selected="true">Railways</option><option value="traffic">Roads</option></select>';
		}
		if (traffic){
			var nav = '<a href="about.html" title="About Page">About</a> | Displaying Current Delays on the National Road Network';
			nav += '&nbsp;&raquo;&nbsp;<select onchange="javascript:goTo(this.options[this.selectedIndex].value);"><option value="weather">Weather</option><option value="rail">Railways</option><option value="traffic" selected="true">Roads</option></select>';
		}
	...
	function reloadRail(){
    	...
    	var request = GXmlHttp.create();
		//var url	= "http://bbc.blueghost.co.uk/includes/rail.php"; //no need for php step anymore
	...
	function goToTraffic(loc, lat, lon, id, title){
		...
		//addon.id	= "traffic_remove_'+loc+'";
		//addon.innerHTML = '<div class="weather_link" id="traffic_remove_'+loc+'"><a href="javascript:removeTraffic(\''+loc+'\');"</div>';
		//var base = document.getElementById('traffic_link_'+loc+'');
		//base.parentNode.insertBefore(addon, base.nextSibling);
		//base.href= 'javascript:removeTraffic(\''+loc+'\', '+lat+', '+lon+', '+marker+');';
		//base.innerHTML = 'Remove Traffic Markers';
	...
	function removeTraffic(loc, lat, lon, id, title){
		...
		for (var i = 0; i < traffic_data.length; i++){
			if (traffic_data[i] == loc){
				...
				//traffic_data[i] = "";
				//alert("Found "+loc+" @ "+index);
			}
		}
		//alert("start = "+traffic_start[index]+", finish = "+traffic_end[index]);
		for (var start = traffic_start[index]; start < traffic_end[index]; start++)
			map.removeOverlay(traffic_markers[start]);
		//var base = document.getElementById('traffic_link_'+loc+'');
		//base.parentNode.insertBefore(addon, base.nextSibling);
		//base.href= 'javascript:goToTraffic(\''+loc+'\', '+lat+', '+lon+');';
		//base.innerHTML = 'Show Traffic Markers';
