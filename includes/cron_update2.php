...
//travel stuff
	...
	$geoRssOutput 	.= "\n<channel rdf:about=\"http://bbc.blueghost.co.uk\">"
					...
					. "\n\t<description>Data from Highways Agency - http://www.highways.gov.uk/. More info at http://bbc.blueghost.co.uk/about_geoRss.html</description>"
					. "\n\t<dc:language>en-gb</dc:language>"
					. "\n</channel>\n";
	
	$output 	= "<traffic_data>";
	$sql 		= 'SELECT * FROM `highways_travel` LIMIT 0, 1000';
	...
	$sql 		= 'SELECT * FROM `highways_travel` ORDER BY `display_name` ASC LIMIT 0, 1000';
	$wl 		= $Database->DB_search($sql);
	while ( $w = $Database->DB_array($wl) )
	{
...
//echo'<h1>Travel Stuff</h1>';
	...
	while ( $w = $Database->DB_array($wl) ){
		...
		$url = "http://www.highways.gov.uk/rssfeed/".$location.".xml";
		copy($url, $dir.'travel_data/'.$location.'_copy.xml');
		$output = eregi_replace("&rtm31_", "", file_get_contents($dir.'travel_data/'.$location.'_copy.xml'));
		$output = eregi_replace("&loc3_", "", $output);
		write_header($dir.'travel_data/'.$location.'_copy.xml', $output);
		$url = $dir.'travel_data/'.$location.'_copy.xml';
		echo'<h4>'.$url.'</h4>';
		$parser = new Highways_Parser($url);
		//$parser->setType("road");
		$err = false;
		if (!$parser->parseFile()){
    		$err = true;
		...
		$geoRssOutput 	.= "\n<channel rdf:about=\"http://bbc.blueghost.co.uk\">"
					...
					. "\n\t<link>http://bbc.blueghost.co.uk</link>"
					. "\n\t<description>Data from Highways Agency - http://www.highways.gov.uk/</description>"
					. "\n\t<dc:language>en-gb</dc:language>"
					. "\n</channel>\n";
