...
function getXML($place, $days){
		...
		if ($days == 0)
		{
			$filename = $dir.'weather_data/search_'.$place.'.xml';
		...
		{
			$filename = $dir.'weather_data/search_'.$place.'_days.xml';
			if (file_exists($filename))
			{
		...
		$sql 	= 'SELECT `id` FROM `weather_locs` WHERE `placename` LIKE ''%'.$place.'%'' LIMIT 1';
		...
		if ($days == 0)
		{
			write_header($dir.'weather_data/search_'.$place.'.xml', $output);
		}
		else
		{
			write_header($dir.'weather_data/search_'.$place.'_days.xml', $output);
		}
		return $output;
	}
