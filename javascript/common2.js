	...
	var sb_site			= 'http://bbc.blueghost.co.uk/site35.php?';
	...
	function reloadWeather(){
		...
		if (day == 0)
			var url		= main_site+'includes/weather_funcs.php?lat_max='+maxY+'&lat_min='+minY+'&lon_max='+maxX+'&lon_min='+minX+'&maximum=100&day=0';
		else
			var url		=  main_site+'includes/weather_funcs.php?lat_max='+maxY+'&lat_min='+minY+'&lon_max='+maxX+'&lon_min='+minX+'&maximum=100&day=1';
	...
	function genPermalink(){
		...
		return p_link = 'type='+type+'&lon='+map.getCenterLatLng().x+'&lat='+map.getCenterLatLng().y+'&zl='+map.getZoom();
	}
	...
	function logoEventHandler(wMarker, logo, reloader, clicker, html){
		...
		var point = new GLatLng(bounds.getNorthEast().lng()-(height*0.87), bounds.getNorthEast().lng()-(width/2));
	...
	function placeCopyLogo(logo, wMarker, img, size_x, size_y, html, move, moveend, clicker, reloader, zoom){
		...
		if (!logo){
			...
			logo.iconAnchor = new GLatLng(0, (size_x/2)); //center img
		}
		//alert('got11');
		var bounds = map.getSize();
		
		var width = bounds.width;
		var height = bounds.height;
		
		var point = new GPoint(width/2, height-100);
